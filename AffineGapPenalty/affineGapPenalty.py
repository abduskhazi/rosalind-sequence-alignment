...
def computeSimilarityMatrix(S1, S2, Mat, ScoringMatrix, gap = lambda g: g):
    ...
    for i in range(1, m+1):
        for j in range(1,n+1):
            ...
            Mat[i][j] = max([ Mat[i-1][j-1] + matchCost,
...
if( __name__ == "__main__"):
    if(len(sys.argv) < 3 ):
        print("usage: python3 affineGapPenalty.py <scoring_matrix_file_name> <data_set_file_name> ")
        sys.exit(-1)
    ...
    P, Q = computeSimilarityMatrix(S1, S2, SimilarityMatrix, scoringMatrix,
                            gapPenalty)
    Alignment_S1, Alignment_S2 = getAlignment(S1, S2, SimilarityMatrix, scoringMatrix, P, Q, gapPenalty)
